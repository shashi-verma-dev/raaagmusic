                    ...
                    <div class="page-title-box">
                        <div class="page-title-right">
                            <ol class="breadcrumb m-0">
                                ...
                                <li class="breadcrumb-item">
                                    <a href="javascript: void(0);">Edit Assets</a>
                                </li>
                            </ol>
                        </div>
                        <h4 class="page-title">Edit Assets</h4>
                    </div>
                </div>
            </div>
...
<form action="{{ route('update_assets', $data->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    <div class="row">
        ...
        <div class="mb-3 col-md-6">
            <label for="stars">Stars (Rating 1-5)</label>
            <input type="number" name="stars" id="stars" class="form-control"
                min="1" max="5" value="{{ old('stars', $data->stars) }}" required>
        </div>

        <div class="mb-3 col-md-6">
            <label for="premium">Premium</label>
            <select name="premium" id="premium" class="form-control" required>
                <option value="0" {{ old('premium', $data->premium) == '0' ? 'selected' : '' }}>Free</option>
                <option value="1" {{ old('premium', $data->premium) == '1' ? 'selected' : '' }}>Premium</option>
            </select>
        </div>
        ...
        <div class="mb-3 col-md-6">
            <label for="og_price">Price (INR)</label>
            <input type="number" name="og_price" id="og_price" class="form-control"
                value="{{ old('og_price', $data->og_price) }}" min="0" required>
        </div>
        ...
        <div class="mb-3 col-md-6">
            ...
            @if($data->thumbnail_image)
                <img src="{{ asset('public/' . $data->thumbnail_image) }}" width="60" class="mt-2">
            @endif
        </div>
        ...
        <div class="mb-3 col-md-6">
            ...
            <input type="file" name="org_file" id="org_file" class="form-control">
            @if($data->org_file)
                <img src="{{ asset('public/' . $data->org_file) }}" width="60" class="mt-2">

                <span class="mt-2"><a href="{{ asset('public/' . $data->org_file) }}" target="_blank">View File</a></span>
            @endif
        </div>

        <div class="mb-3 col-md-6">
            <label for="org_file_without_bg">Original File (Without Background)</label>
            <input type="file" name="org_file_without_bg" id="org_file_without_bg" class="form-control">
            @if($data->org_file_without_bg)
                <img src="{{ asset('public/' . $data->org_file_without_bg) }}" width="60" class="mt-2">

                <span class="mt-2"><a href="{{ asset('public/' . $data->org_file_without_bg) }}" target="_blank">View File</a></span>
            @endif
        </div>
        ...
        <div class="mb-3 col-md-6">
            <label for="resolution">Resolution (e.g. 1920x1080)</label>
            <input type="text" name="resolution" id="resolution" class="form-control"
                value="{{ old('resolution', $data->resolution) }}" required>
        </div>
        ...
        <div class="mb-3 col-md-6">
            ...
            <input type="file" name="zip_file" id="zip_file" class="form-control">
            @if($data->zip_file)
                <p class="mt-2"><a href="{{ asset('public/' . $data->zip_file) }}" target="_blank">Download Zip</a></p>
            @endif
        </div>

        <div class="mb-3 col-md-6">
            <label for="zip_file_size">Zip File Size (in KB/MB)</label>
            <input type="text" name="zip_file_size" id="zip_file_size" class="form-control"
                value="{{ old('zip_file_size', $data->zip_file_size) }}">
        </div>
        ...
        <div class="mb-3 col-md-6">
            <label for="tags">Tags (comma-separated)</label>
            <textarea name="tags" id="tags" class="form-control" rows="1" required>{{ old('tags', $data->tags) }}</textarea>
        </div>

        <div class="mb-3 col-md-12">
            <label for="description">Description</label>
            <textarea name="description" id="description" class="form-control" rows="3" required>{{ old('description', $data->description) }}</textarea>
        </div>
